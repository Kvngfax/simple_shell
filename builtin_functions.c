#include "shell.h"
int (*get_builtin(char *command))(char **args, char **beginning);
int process_exit(char **args, char **beginning);
int process_cd(char **args, char __attribute__((__unused__)) **beginning);
int process_help(char **args, char __attribute__((__unused__)) **beginning);

/**
 * get_builtin - Retrieves the function pointer corresponding
 * 				 to a shellby builtin command.
 * @command: The command to match.
 * Return: A function pointer to the corresponding builtin.
 */
int (*get_builtin(char *command))(char **args, char **beginning)
{
	builtin_t funcs[] = {
		{ "exit", process_exit },
		{ "env", process_env },
		{ "setenv", process_setenv },
		{ "unsetenv", process_unsetenv },
		{ "cd", process_cd },
		{ "alias", process_alias },
		{ "help", process_help },
		{ NULL, NULL }
	};
	int i;

	for (i = 0; funcs[i].name; i++)
	{
		if (_strcmp(funcs[i].name, command) == 0)
			break;
	}
	return (funcs[i].func);
}

/**
 * process_exit - Causes normal process termination
 *                for the shellby shell.
 * @args: An array of arguments containing the exit value.
 * @beginning: A double pointer to the beginning of args.
 *
 * Return: If there are no arguments - -3.
 *         If the given exit value is invalid - 2.
 *         O/w - exits with the given status value.
 *
 * Description: Upon returning -3, the program exits back in the main function.
 */
int process_exit(char **args, char **beginning)
{
	int i, len_of_int = 10;
	unsigned int num = 0, max = 1 << (sizeof(int) * 8 - 1);

	if (args[0])
	{
		if (args[0][0] == '+')
		{
			i = 1;
			len_of_int++;
		}
		for (; args[0][i]; i++)
		{
			if (i <= len_of_int && args[0][i] >= '0' && args[0][i] <= '9')
				num = (num * 10) + (args[0][i] - '0');
			else
				return (create_error(--args, 2));
		}
	}
	else
	{
		return (-3);
	}
	if (num > max - 1)
		return (create_error(--args, 2));
	args -= 1;
	free_args(args, beginning);
	clean_env();
	free_alias_list(aliases);
	exit(num);
}

/**
 * process_cd - Changes the current working directory
 * 			 for the shellby process.
 * @args: An array of arguments.
 * @beginning: A double pointer to the beginning of args.
 *
 * Return: If the given string is not a directory - 2.
 *         If an error occurs - -1.
 *         Otherwise - 0.
 */
int process_cd(char **args, char __attribute__((__unused__)) **beginning)
{
	char **dir_info, *new_line = "\n";
	char *oldpwd = NULL, *pwd = NULL;
	struct stat dir;

	oldpwd = getcwd(oldpwd, 0);
	if (!oldpwd)
		return (-1);

	if (args[0])
	{
		if (*(args[0]) == '-' || _strcmp(args[0], "--") == 0)
		{
			if ((args[0][1] == '-' && args[0][2] == '\0') ||
					args[0][1] == '\0')
			{
				if (retrieve_env("OLDPWD") != NULL)
					(chdir(*retrieve_env("OLDPWD") + 7));
			}
			else
			{
				free(oldpwd);
				return (create_error(args, 2));
			}
		}
		else
		{
			if (stat(args[0], &dir) == 0 && S_ISDIR(dir.st_mode)
					&& ((dir.st_mode & S_IXUSR) != 0))
				chdir(args[0]);
			else
			{
				free(oldpwd);
				return (create_error(args, 2));
			}
		}
	}
	else
	{
		if (retrieve_env("HOME") != NULL)
			chdir(*(retrieve_env("HOME")) + 5);
	}

	pwd = getcwd(pwd, 0);
	if (!pwd)
		return (-1);

	dir_info = malloc(sizeof(char *) * 2);
	if (!dir_info)
		return (-1);

	dir_info[0] = "OLDPWD";
	dir_info[1] = oldpwd;
	if (process_setenv(dir_info, dir_info) == -1)
		return (-1);

	dir_info[0] = "PWD";
	dir_info[1] = pwd;
	if (process_setenv(dir_info, dir_info) == -1)
		return (-1);
	if (args[0] && args[0][0] == '-' && args[0][1] != '-')
	{
		write(STDOUT_FILENO, pwd, _strlen(pwd));
		write(STDOUT_FILENO, new_line, 1);
	}
	free(oldpwd);
	free(pwd);
	free(dir_info);
	return (0);
}

/**
 * process_help - Displays information about shellby builtin commands.
 * @args: An array of arguments.
 *
 * @beginning: A pointer to the beginning of args.
 *
 * Return: If an error occurs - -1.
 *         Otherwise - 0.
 */
int process_help(char **args, char __attribute__((__unused__)) **beginning)
{
	if (!args[0])
		display_all_help();
	else if (_strcmp(args[0], "alias") == 0)
		display_alias_help();
	else if (_strcmp(args[0], "cd") == 0)
		display_cd_help();
	else if (_strcmp(args[0], "exit") == 0)
		display_exit_help();
	else if (_strcmp(args[0], "env") == 0)
		display_help_env();
	else if (_strcmp(args[0], "setenv") == 0)
		display_help_setenv();
	else if (_strcmp(args[0], "unsetenv") == 0)
		display_help_unsetenv();
	else if (_strcmp(args[0], "help") == 0)
		display_help_cmd();
	else
		write(STDERR_FILENO, name, _strlen(name));

	return (0);
}
